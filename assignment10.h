//
// Created by asus on 4/1/2023.
//

#ifndef ASSIGNMENT6_C_ASSIGNMENT10_H
#define ASSIGNMENT6_C_ASSIGNMENT10_H
...
int get_limit_amount(int user_index);
unsigned int calculate_amounts_same_days_transmitter(int to_calcu_index);
unsigned int calculate_amounts_same_days_receiver(int to_calcu_index);
...
void transfer_money(int transfer , int receiver , unsigned int amount){
    ...
    current_data_toTransfer(amount);


    printf("loading to transfer.....\n");
    unsigned int total_amount_transmitter = calculate_amounts_same_days_transmitter(transfer);
    unsigned int total_amount_receiver = calculate_amounts_same_days_receiver(receiver);
    printf("\nName: %s : total_amount: %u\n",db[transfer].name,total_amount_transmitter);
    printf("\nName: %s : total_amount: %u\n",db[transfer].name,total_amount_receiver);
    int trans_limit_transmitter=get_limit_amount(transfer);
    int trans_limit_receiver = get_limit_amount(receiver);
...
unsigned int calculate_amounts_same_days_transmitter(int to_calcu_index){
    ...
    unsigned int total_amount_for_same_day_transmitter=0;
    ...
    for(int a=record_counter-1; a>=0 ; a--){
        ...
        unsigned long long int current_record_day= charArray_to_unsigned_fun(day_char_array);
        ...
        // to add transfer or receiver  // char from[5]={'F','r','o','m','-'};
        ...
        char from[5]={'F','r','o','m','-'};
        for (int i=0; i<6; i++){
            if(db[to_calcu_index].trc[a].note[i]==from[i]){
                transimition++;
            }
        }
        if(transimition==4){
            total_amount_for_same_day_transmitter = total_amount_for_same_day_transmitter+current_record_amount;
        }
    ...
    printf(" total amount for same day! %u",total_amount_for_same_day_transmitter);

    return total_amount_for_same_day_transmitter;
}
...
unsigned int calculate_amounts_same_days_receiver(int to_calcu_index){
    ...
    unsigned int total_amount_for_same_day_receiver=0;
    ...
    for(int a=record_counter-1; a>=0 ; a--){
        ...
        if(receiable>-1){
            total_amount_for_same_day_receiver = total_amount_for_same_day_receiver+current_record_amount;
        }
    ...
    printf(" total amount for same day! %u",total_amount_for_same_day_receiver);

    return total_amount_for_same_day_receiver;
}
...
#endif //ASSIGNMENT6_C_ASSIGNMENT10_H
